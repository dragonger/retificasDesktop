#!/bin/bash
# Liga/desliga o serviço retifica-backend via API do Railway, seguindo o
# horário de funcionamento da oficina (ver crontab gerado no Dockerfile).
# Baseado no template github.com/smolpaw/railway-service-cron (MIT),
# adaptado aqui só pra suportar horários diferentes por dia da semana
# (ex.: sábado fecha mais cedo que os dias úteis) — o template original só
# aceita um único horário de abrir e um de fechar pra semana inteira.
...
shutdown_service() {
  ...
  if [ "$deployment_status" = "REMOVED" ]; then
    echo "Service already stopped. Skipping..."
    echo ""
    return 0
  fi
...
deploy_service() {
  ...
  if [ "$deployment_status" != "SUCCESS" ] && [ "$deployment_status" != "REMOVED" ]; then
    echo "Service not ready for deployment. Skipping deployment..."
    echo ""
    return 0
  fi
